...
router.post('/', upload.array('images', 5), async (req, res) => {
  try {
    ...
    let sizeStock = {};
    if (req.body.sizeStock) { try { sizeStock = JSON.parse(req.body.sizeStock); } catch(e) {} }
    ...
    const product = new Product({
      ...
      price: req.body.price, originalPrice: req.body.originalPrice || undefined,
      image: imageUrls[0] || '', images: imageUrls, label: req.body.label || '',
      sizes, sizeStock, inStock
    });
    await product.save();
...
router.put('/:id', upload.array('images', 5), async (req, res) => {
  try {
    ...
    const updatedProduct = await Product.findByIdAndUpdate(req.params.id, {
      ...
      price: req.body.price || product.price, originalPrice: req.body.originalPrice || product.originalPrice,
      image: imageUrls[0] || product.image, images: imageUrls,
...
router.post('/import', async (req, res) => {
  try {
    ...
    let sizeList = Array.isArray(sizes) ? sizes : (sizes ? String(sizes).split(',').map(s => s.trim()).filter(Boolean) : []);
    const availableRaw = String(req.body.available || '0').trim();
    const sizeStock = {};
    if (availableRaw.includes(':')) {
      availableRaw.split(';').forEach(part => { const [s, q] = part.split(':').map(x => x.trim()); if (s) sizeStock[s] = parseInt(q) || 0; });
    ...
    if (existing) {
      const updatedProduct = await Product.findByIdAndUpdate(existing._id, {
        category: category ? category.toLowerCase() : existing.category,
        subcategory: subcategory || existing.subcategory, price: Number(price),
        ...
        sizeStock: sizeList.length ? sizeStock : Object.fromEntries(existing.sizeStock || new Map()), inStock
      }, { new: true });
      return res.json({ success: true, updated: true, product: updatedProduct });
    }
    const product = new Product({
      name, category: category ? category.toLowerCase() : 'mens', subcategory: subcategory || '',
      price: Number(price), originalPrice: originalPrice ? Number(originalPrice) : undefined,
      image: '', images: [], label: label || '', sizes: sizeList, sizeStock, inStock
    });
    await product.save();
...
router.post('/reorder', async (req, res) => {
  try {
    ...
    for (const item of order) { await Product.findByIdAndUpdate(item.id, { position: item.position }); }
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
